...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = current_schema() AND table_name = 'user_logs' AND column_name = 'start'
  ) THEN
    UPDATE user_logs SET start_time = start WHERE start_time IS NULL AND start IS NOT NULL;
    UPDATE user_logs SET end_time = "end" WHERE end_time IS NULL AND "end" IS NOT NULL;
